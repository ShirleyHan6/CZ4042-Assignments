...
from models import char_rnn_model
...
logits = char_rnn_model(x, keep_prob)
...
with tf.Session() as sess:
  ...
  for e in range(epochs):
    ...
    for start in range(0, N-batch_size, batch_size):
      train_op.run(feed_dict={x: x_train[start: start+batch_size], y_: y_train[start: start+batch_size], keep_prob:1.0})
...
with open('../output/char_rnn_0.4.pkl', 'wb') as f:
